...
po.map = function() {
  ...
  mapCanvas.style['pointer-events'] = 'none';
	var mapRender = PIXI.autoDetectRenderer(800,600,{view:mapCanvas,transparent:true});
	var mapRootContainer = new PIXI.Container();
	mapRootContainer.interactiveChildren = false;
	map.rootContainer = function() {
		return mapRootContainer;
	}
	map.renderMap = function() {
		mapRender.render(mapRootContainer);
	}
  ...
  map.locationPoint = function(l) {
	...
	// YGQ
  map.locationPointArray = function(l) {
    var k = Math.pow(2, zoom + zoomFraction - 3) / 45,
        dx = (l[0] - center.lon) * k * tileSize.x,
        dy = (lat2y(center.lat) - lat2y(l[1])) * k * tileSize.y;
    return {
      x: sizeRadius.x + angleCos * dx - angleSin * dy,
      y: sizeRadius.y + angleSin * dx + angleCos * dy
    };
  ...
  map.pointLocationLL = function(p) {
    var k = 45 / Math.pow(2, zoom + zoomFraction - 3),
        dx = (p.x - sizeRadius.x) * k,
        dy = (p.y - sizeRadius.y) * k;
    p.lon = center.lon + (angleCosi * dx - angleSini * dy) / tileSize.x;
    p.lat = y2lat(lat2y(center.lat) - (angleSini * dx + angleCosi * dy) / tileSize.y);
  };
  ...
  map.container = function(x) {
    ...
    // YGQ
    container.parentNode.insertBefore(mapCanvas, container);
  ...
  map.resize = function() {
    ...
    mapCanvas.setAttribute("width", sizeActual.x);
    mapCanvas.setAttribute("height", sizeActual.y);
    mapRender.resize(sizeActual.x,sizeActual.y);
...
po.layer = function(load, unload) {
  var layer = {},
      cache = layer.cache = po.cache(load, unload).size(2048),
      tile = true,
      visible = true,
      zoom,
      id,
  ...
  function move() {
    ...
    for (var key in newLocks) {
      ...
      // YGQ
			var spriteScale = Math.pow(2, mapZoom-t.zoom + mapZoomFraction);
	
      if (t.sprite){
	      t.sprite.x = mapSize.x/2 + tileSize.x * (t.column - tileCenter.column * k) * spriteScale;
      	t.sprite.y = mapSize.y/2 + tileSize.y * (t.row - tileCenter.row * k) * spriteScale;
      	t.sprite.scale.x = t.sprite.scale.y = spriteScale;
      	if (t.ready)
    ...
    if (e.tile.proxyRefs) {
      for (var proxyKey in e.tile.proxyRefs) {
        ...
        if ((--proxyTile.proxyCount <= 0) && cache.unload(proxyKey)) {
          // YGQ
          if (proxyTile.sprite)
          	proxyTile.sprite.visible = false;
        }
      }
...
// YGQ: add svgLayer for geojson
po.svglayer = function(load, unload) {
  var layer = {},
...
po.image = function() {
  ...
  function load(tile) {
    ...
    if (typeof url == "function") {
      ...
      if (tileUrl != null) {
        ...
        tile.visible = false;
        image.map().rootContainer().addChild(tile.sprite);
        tile.request = po.queue.image(tileUrl, function(img) {
          delete tile.request;
          tile.ready = true;
          tile.img = img;
	        //YGQ
	        tile.sprite.texture = new PIXI.Texture(new PIXI.BaseTexture(img));
  ...
  function unload(tile) {
    ...
    if (tile.sprite) {
    	...
    	image.map().rootContainer().removeChild(tile.sprite);
    }
  }
...
po.geoJson = function(fetch) {
  var geoJson = po.svglayer(load, unload),
...
po.drag = function() {
  ...
  function mousemove(e) {
    ...
    if (newTime-lastMoveTime<25 && e.type!='mouseup') return;
  ...
  function mouseup(e) {
    if (!dragging) return;
    let d = {x: e.clientX - dragging.x, y: e.clientY - dragging.y};

		// YGQ: 添加鼠标点击事件
    var newTime = new Date();
    if (newTime-lastDownTime<300 && e.clientX==dragging.x && e.clientY==dragging.y)
    	map.dispatch({type: "click"});
